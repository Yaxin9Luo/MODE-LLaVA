...
class OpenCLIPVisionTower(nn.Module):
    ...
    def load_model(self):
        ...
        load_checkpoint(clip_model, ckpt_path, strict=True)
...
def load_checkpoint(model, checkpoint_path, strict=True):
    ...
    if is_deepspeed_zero3_enabled():
        ...
        # Filter out text-related keys and logit_scale
        vision_state_dict = {k: v for k, v in state_dict.items() 
                            if not k.startswith('text.') and k != 'logit_scale'}
        # incompatible_keys = model.load_state_dict(state_dict, strict=strict)
        incompatible_keys = model.load_state_dict(vision_state_dict, strict=strict)
