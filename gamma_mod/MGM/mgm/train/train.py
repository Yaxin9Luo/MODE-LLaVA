...
from mgm.model.language_model.mod_mgm_llama import MoDMGMLlamaForCausalLM
local_rank = None
...
class ModelArguments:
    ...
    mod_layers_idx: Optional[List[int]] = field(default=None, metadata={"help": "where to place mod layers."})
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        for idx, item in enumerate(list_data_dict):
            if 'dataset' in item:
                self.dataset_info[idx] = item['dataset']
            else:
                self.dataset_info[idx] = 'unknown'
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        while attempt < max_attempt:
            try:
                ...
                attempt += 1
                print(f"Error in loading {i} from dataset {self.dataset_info[original_i]}, retrying...")
                print(f"Error message: {str(e)}")
                i = random.randint(0, len(self.list_data_dict)-1)
        if attempt == max_attempt:
            print(f"Failed to load item after {max_attempt} attempts. Original index: {original_i}, Dataset: {self.dataset_info[original_i]}")
            # except:
            #     attempt += 1
    ...
    if model_args.vision_tower is not None:
        if "mistral" in  model_args.model_name_or_path.lower():
            ...
            if model_args.mod_enable:
                ...
                model = MoDMGMLlamaForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                attn_implementation=attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),   
