...
from gamma_mod.mm_utils import tokenizer_image_token, process_images, get_model_name_from_path
...
def str2box(strings):
    ...
    if result is None:
        print(strings)
        return [0., 0., 0., 0.]
    match = re.search(r'\[(.*?)\]', strings)
    string_list=match.group()
    if re.fullmatch(r'^[\d\s,.\[\]]*$', string_list):
        pred_box = ast.literal_eval(string_list)
    else:
        pred_box = [0., 0., 0., 0.]
    return  pred_box

# def str2box(input_str):
#     match = re.match(r"\[\s*'([0-9]*(\.[0-9]*)?)'(,\s*'([0-9]*(\.[0-9]*)?)')*\s*\]", input_str)
#     if match:
#         str_list = ast.literal_eval(input_str)
...
def eval_model(args):
    ...
    for (input_ids, image_tensor), line in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        outputs = outputs.strip()
        pred=str2box(outputs)

        ans_file.write(json.dumps({"ans": ans,
                                   "pred": pred,
                                   "model_id": model_name,
                                   "metadata": {}}) + "\n")
