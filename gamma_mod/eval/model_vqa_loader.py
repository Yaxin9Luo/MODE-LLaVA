...
def eval_model(args):
    ...
    max_samples = 20000
    for i, ((input_ids, image_tensor), line) in enumerate(tqdm(zip(data_loader, questions), total=min(len(questions), max_samples))):
        if i >= max_samples:
            break
        total_samples += 1
    ...
    print('Tokens per second: ', float(total_num_dec_tokens) / total_inference_time)
